...
func TestTestHostHTTPAndResolve(t *testing.T) {
	host := NewTestHost(func(b *di.Builder) {
		di.RegisterInstance[*testService](b, &testService{Name: "svc"})
	}, func(app *web.Server, container *di.Container) error {
		app.MapGet("/ping", func(ctx *web.Context) error {
			return ctx.JSON(201, map[string]string{"ok": "yes"})
		})
		app.MapPost("/echo", func(ctx *web.Context) error {
...
func TestNewTestHostClosesContainerWhenRegistrationFails(t *testing.T) {
	tracker := &closeTracker{}
	defer func() {
		if recover() == nil {
			t.Fatal("NewTestHost() did not panic")
		}
		if !tracker.closed {
			t.Fatal("NewTestHost() leaked the built container")
		}
	}()

	NewTestHost(func(builder *di.Builder) {
		di.RegisterInstance[io.Closer](builder, tracker)
	}, func(*web.Server, *di.Container) error {
