...
type reentrantGuardFSM struct {
	State   string `fsm:"initial:idle; idle->active"`
	machine *Machine
}

func (f *reentrantGuardFSM) CanActive() error {
	if f.machine.CurrentState() != "idle" {
		return errGuardType("unexpected state")
	}
	return nil
}
...
func TestTransition_GuardCanReadMachine(t *testing.T) {
	state := &reentrantGuardFSM{}
	machine, err := New(state)
	if err != nil {
		t.Fatal(err)
	}
	...
	go func() {
		done <- machine.Transition("active")
	}()
	select {
	case err := <-done:
		if err != nil {
	...
	case <-time.After(time.Second):
		t.Fatal("transition guard deadlocked while reading machine state")
	}
}
...
func TestBeforeTransitionListenerSeesSourceState(t *testing.T) {
	order := &orderFSM{}
	machine, err := New(order)
	if err != nil {
		t.Fatal(err)
	}
	var observed string
	machine.Subscribe(func(event Event) {
		if event.Type == BeforeTransition {
			observed = event.Machine.CurrentState()
		}
	})

	if err := machine.Transition("confirmed"); err != nil {
		t.Fatal(err)
	}
	if observed != "draft" {
		t.Fatalf("BeforeTransition observed state %q, want draft", observed)
	}
}
