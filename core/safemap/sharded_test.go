...
func TestShardedMapRemovesExpiredEntriesFromViews(t *testing.T) {
	mapping := NewSharded[string, int](StringHasher, 4).WithExpiry(time.Millisecond)
	mapping.Set("expired", 1)
	time.Sleep(2 * time.Millisecond)
	if length := mapping.Len(); length != 0 {
		t.Fatalf("Len() = %d, want 0", length)
	}
	if keys := mapping.Keys(); len(keys) != 0 {
		t.Fatalf("Keys() = %v, want empty", keys)
	}
	if values := mapping.Values(); len(values) != 0 {
		t.Fatalf("Values() = %v, want empty", values)
	}
}

func TestShardedMapExpiryConfigurationIsConcurrent(t *testing.T) {
	mapping := NewSharded[string, int](StringHasher, 4)
	var wait sync.WaitGroup
	for index := 0; index < 100; index++ {
		wait.Add(2)
		go func() {
			defer wait.Done()
			mapping.WithExpiry(time.Millisecond)
		}()
		go func(value int) {
			defer wait.Done()
			mapping.Set(strconv.Itoa(value), value)
		}(index)
	}
	wait.Wait()
}

func TestNewShardedRejectsNilHasher(t *testing.T) {
	defer func() {
		if recover() == nil {
			t.Fatal("NewSharded() accepted a nil hasher")
		}
	}()
	NewSharded[string, int](nil, 4)
}

func TestShardedMapDeleteHashesOnce(t *testing.T) {
	var calls int
	mapping := NewSharded[string, int](func(string) uint64 {
		calls++
		return uint64(calls)
	}, 4)
	mapping.Delete("key")
	if calls != 1 {
		t.Fatalf("Delete() called the hasher %d times, want 1", calls)
	}
}
