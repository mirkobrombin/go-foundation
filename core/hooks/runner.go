	...
	for _, fn := range all {
		...
		go func() {
			...
			case <-ctx.Done():
				errCh <- ctx.Err()
			default:
				errCh <- callHook(ctx, key, args, fn)
			}
		}()
	}
...
func callHook(ctx context.Context, key string, args []any, fn HookFunc) (err error) {
	if fn == nil {
		return errors.New("hooks: hook cannot be nil")
	}
	defer func() {
		if recovered := recover(); recovered != nil {
			err = fmt.Errorf("hooks: hook panic: %v", recovered)
		}
	}()
	return fn(ctx, key, args)
}
