...
func TestBinder_BindJSONRejectsUnknownField(t *testing.T) {
	type Inner struct {
		Name string `json:"name"`
	}
	type Target struct {
	...
	var target Target
	err := New().BindJSON(&target, []byte(`{"name":"bob","admin":true}`))
	if err == nil {
		t.Fatal("BindJSON() accepted an unknown field")
	}
}

func TestBinder_BindJSONRejectsTrailingValue(t *testing.T) {
	type Inner struct {
		Name string `json:"name"`
	}
	type Target struct {
	...
	var target Target
	err := New().BindJSON(&target, []byte(`{"name":"bob"} {"name":"eve"}`))
	if err == nil {
		t.Fatal("BindJSON() accepted multiple JSON values")
	}
}
