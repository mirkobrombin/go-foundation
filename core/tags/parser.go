...
func (p *Parser) ParseType(typ reflect.Type) []FieldMeta {
	...
	fields := p.parseTypeRecursive(typ, "", nil)
	p.cache[typ] = fields
	return fields
}

func (p *Parser) parseTypeRecursive(typ reflect.Type, prefix string, parentIndex []int) []FieldMeta {
	var fields []FieldMeta

	for i := 0; i < typ.NumField(); i++ {
		field := typ.Field(i)
		indexPath := append(append([]int(nil), parentIndex...), i)
		...
		if shouldRecurse {
			...
			nested := p.parseTypeRecursive(nestedType, fieldName, indexPath)
