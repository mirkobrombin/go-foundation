...
func (p *Policy) Unmarshal(data []byte, v any) error {
	...
	if target.Kind() != reflect.Ptr || target.IsNil() {
		return errors.New("serializer: destination must be a non-nil pointer")
	}
...
func (p *Policy) toMap(v any) (map[string]any, error) {
	...
	if !rv.IsValid() {
		return nil, errors.New("serializer: cannot marshal nil")
	}
	for rv.Kind() == reflect.Ptr {
		if rv.IsNil() {
			return nil, errors.New("serializer: cannot marshal nil pointer")
		}
		rv = rv.Elem()
	}
	...
	for i := 0; i < rt.NumField(); i++ {
		...
		name, skip := p.fieldName(field)
		if skip {
			continue
		}
		encoded, err := p.encodeValue(fv)
		if err != nil {
			return nil, fmt.Errorf("serializer: field %s: %w", field.Name, err)
		}
		result[name] = encoded
	}
	return result, nil
...
func (p *Policy) fromMap(raw map[string]any, v any) error {
	...
	for i := 0; i < rt.NumField(); i++ {
		...
		if val, ok := raw[name]; ok {
			fv := elem.Field(i)
			if fv.CanSet() {
				if err := p.decodeValue(fv, val); err != nil {
...
func (p *Policy) fieldName(field reflect.StructField) (string, bool) {
	...
	return transformName(field.Name, p.naming), false
}
...
func (p *Policy) encodeValue(value reflect.Value) (any, error) {
	p.mu.RLock()
	factory, ok := p.customTypes[value.Type()]
	p.mu.RUnlock()
	if !ok {
		return value.Interface(), nil
	}
	if factory == nil {
		return nil, errors.New("custom converter factory is nil")
	}
	converter := factory()
	if converter == nil {
		return nil, errors.New("custom converter is nil")
	}
	return converter.Encode(value.Interface())
}

func (p *Policy) decodeValue(field reflect.Value, value any) error {
	p.mu.RLock()
	factory, ok := p.customTypes[field.Type()]
	p.mu.RUnlock()
	if ok {
		if factory == nil {
			return errors.New("custom converter factory is nil")
		}
		converter := factory()
		if converter == nil {
			return errors.New("custom converter is nil")
		}
...
func setValue(fv reflect.Value, val any) error {
	if val == nil {
		switch fv.Kind() {
		case reflect.Interface, reflect.Map, reflect.Pointer, reflect.Slice:
			...
			return nil
		default:
			return errors.New("cannot assign null")
		}
	}
...
func isSafeConversion(from, to reflect.Kind) bool {
	...
	return from == reflect.String && to == reflect.String
}
