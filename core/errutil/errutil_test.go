...
func TestAutoHTTPDoesNotExposePanicOrRequestSecrets(t *testing.T) {
	handler := AutoHTTP(http.HandlerFunc(func(http.ResponseWriter, *http.Request) {
		panic("database-password")
	}))
	request := httptest.NewRequest(
		http.MethodGet,
		"https://example.com/path?token=request-secret",
		nil,
	)
	...
	for _, secret := range []string{"database-password", "request-secret", "token="} {
