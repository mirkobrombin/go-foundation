...
func drawRichTrace(r any) {
	...
	for i := 0; i < len(lines); i++ {
		...
		if strings.Contains(t, "core/errutil/") || strings.HasPrefix(t, "created by ") {
			continue
		}
		...
		if !strings.Contains(t, ".go:") && !strings.HasPrefix(t, "created by ") {
			...
			if !strings.Contains(e.funcName, "drawRichTrace") && !strings.Contains(e.funcName, "Auto.func1") && !strings.HasPrefix(e.funcName, "Auto") && !strings.HasPrefix(e.funcName, "EnableAuto") && !strings.HasPrefix(e.funcName, "github.com/mirkobrombin/go-foundation/v2/core/errutil.") {
